...
for i in range(y_axis_range + 1):
    ...
    for j in range(x_axis_range + 1):
        temp.append(' ≈≈≈≈ ')
    ocean.append(temp)
for i in range(0,y_axis_range + 1):
    ocean[0][i] = x_axis[i]
...
for i in range(x_axis_range):
    temp = []
    for j in range(y_axis_range):
        target_dict[i+1,j+1] = target[i][j]
        vocab_target_dict.append([i+1,j+1])
